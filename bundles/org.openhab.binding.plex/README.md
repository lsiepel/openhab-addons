...
1. In the URL, locate the token in the X-Plex-Token parameter.
...
To add a player manually, go to <https://plex.tv/devices.xml> and sign in if needed.
...
```java
Number    BridgePlexCurrent            "Current players"           { channel="plex:server:plexrServer:currentPlayers" }
Number    BridgePlexCurrentActive      "Current players active"    { channel="plex:server:plexrServer:currentPlayersActive" }
